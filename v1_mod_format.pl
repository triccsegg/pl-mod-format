...
open OUTF, ">:encoding(UTF-8)", "$output_file_phase"  or die "can't open file $output_file_phase" ;
  ...
  {	  
	...
	if(m!^(.+?)(<a\s*href\s*=\s*"https://\d+\.bp\.blogspot\.com.*)!)
	{
	  #print OUTF "line:$_\n";
      my $head_match;
	  my $tail_match;
	  
	  while(m!^(.+?)(<a\s*href\s*=\s*"https://\d+\.bp\.blogspot\.com.*)!) #ensure <a href ... is first character
	  {
	    $head_match = $1;
	    $tail_match = $2;
	  
        $head_match =~ s!(<a\s*href\s*=\s*"https://\d+\.bp\.blogspot\.com.*?</a>)(\s*)(\S+)!$1\n$3!g; #<a href...</a>XXX => <a href...</a>\nXXX
	    print OUTF "$head_match\n";
	    $_ = $tail_match;
	  }
	    
	  if($tail_match =~ m!(<a\s*href\s*=\s*"https://\d+\.bp\.blogspot\.com.*?</a>)(\s*)(\S+)!) #<a href...</a>XXX => <a href...</a>\nXXX
	  {
        print OUTF "$1\n$3\n";
	  }
	  else
...
open OUTF, ">:encoding(UTF-8)", "$output_file_phase"  or die "can't open file $output_file_phase" ;
  ...
  {
    chomp;
	s!(<a\s*href\s*=\s*"https://\d+\.bp\.blogspot\.com.*?\.(JPG|jpg|png|PNG)").*?>\s*(<img.*?/>\s*</a>)!$1>$3!g;
    print OUTF "$_\n";
  }
